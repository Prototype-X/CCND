...
"""
For Dlink DES-3028, DES-3528, DES-3552, DGS-3426G, DGS-3627G
...
class Dlink(Telnet):
    def __init__(self):
        ...
        self.prompt = re.compile(r'Upload configuration\.*Done\.*')
        self.cmd_backup = ''

    def run_cmd(self):
        self.cmd_backup = 'upload cfg_toTFTP {} dest_file {}.cfg\n'.format(self.cfg_device['tftp_ip'],
                                                                           self.cfg_device['name'])
        try:
            self.telnet.write(self.cmd_backup)
            self.telnet.expect([self.prompt, pexpect.TIMEOUT], timeout=20)
